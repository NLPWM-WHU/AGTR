...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim, sequence_length, user_num, item_num,n_latent,step_size=4,
                 learning_rate=0.01, reward_gamma=0.95, start_token=0):
        ...
        with tf.name_scope("u_i_his_embedding"):   #users/items history reviews emb
            ...
            self.u_feas_his = tf.nn.embedding_lookup(self.uidW, self.input_uid)
            self.i_feas_his = tf.nn.embedding_lookup(self.iidW, self.input_iid)
            ...
            self.gen_input = tf.nn.tanh(tf.matmul(tf.concat([self.u_feas_his, self.i_feas_his], 1), self.u_i_to_gen_W) + self.u_i_to_gen_b)
            self.g_params.extend([self.u_i_to_gen_W, self.u_i_to_gen_b])
    ...
    def pretrain_step(self, sess, x, uid, iid):
        outputs = sess.run([self.pretrain_updates, self.pretrain_loss], feed_dict={self.x: x, self.input_uid:uid, self.input_iid:iid})
        return outputs[1]
