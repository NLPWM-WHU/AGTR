...
class Discriminator(object):
    def __init__(self, sequence_length, num_classes, vocab_size,dis_emb_dim,filter_sizes, num_filters,batch_size,hidden_dim,
        ...
        with tf.name_scope('D_update'):
            ...
            with tf.variable_scope("feature") as self.feature_scope:
                D_feature = self.FeatureExtractor_unit(self.D_input_x,self.input_uid, self.input_iid, self.dropout_keep_prob)#,self.dropout_keep_prob)
    ...
    def concat_word_u_i_vec(self, u_vecs, i_vecs, emb_revs):
        emb_revs = tf.unstack(emb_revs, num=self.sequence_length, axis=1)
        for i in range(self.sequence_length):
            emb_revs[i] = tf.concat([emb_revs[i], u_vecs, i_vecs], 1)
    ...
    def FeatureExtractor(self):
        ...
        def unit(Feature_input_x, input_uid, input_iid, dropout_keep_prob):#,dropout_keep_prob):
            with tf.variable_scope('FeatureExtractor') as scope:
                with tf.device('/cpu:0'), tf.name_scope("embedding") as scope:
                    ...
                    D_input_con = self.concat_word_u_i_vec(u_vecs, i_vecs, embedded_revs)
                ...
                for filter_size, num_filter in zip(self.filter_sizes, self.num_filters):
                    with tf.name_scope("conv-maxpool-%s" % filter_size) as scope:
                        # Convolution Layer
                        filter_shape = [filter_size, self.dis_emb_dim+self.n_latent*2, 1, num_filter]
