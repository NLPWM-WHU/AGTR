...
class Generator(object):
    def __init__(self, sequence_length, num_vocabulary,
                 ...
                 goal_size, step_size, D_model, rev_num_u, rev_len_u, rev_num_i, rev_len_i,item_num, user_num, n_latent,
                 user_vocab_size, word_emb_size, item_vocab_size,num_filters_his, LSTMlayer_num=1,
                 l2_reg_lambda=0.0, learning_rate=0.01, drop_out=1):
        ...
        with tf.name_scope("u_i_his_embedding"):   #users/items history reviews emb
            ...
            self.gen_input = tf.nn.relu(tf.matmul(tf.concat([self.u_feas_his_drop, self.i_feas_his_drop], 1), self.u_i_to_gen_W) + self.u_i_to_gen_b)
        ...
        # 将 原本的start token作为生成器的输入 改为用u_feas,i_feas拼接结果
        _, _, _, _, self.gen_o, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       self.gen_input, self.h0_worker, self.h0_manager,
        ...
        _, _, self.g_predictions, _, _, _, _, _, self.feature_array, self.real_goal_array, self.sub_feature, self.all_sub_features, self.all_sub_goals = control_flow_ops.while_loop(
            ...
            body=preTrain,
            loop_vars=(
            tf.constant(0, dtype=tf.int32), self.gen_input, g_predictions,self.h0_worker,
    ...
    def rollout(self, input_x, given_num):
        ...
        i, x_t, _, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward, given_num = control_flow_ops.while_loop(
            ...
            loop_vars=(
            tf.constant(0, dtype=tf.int32), self.gen_input, self.x,
            gen_for_reward,
    ...
    def pretrain_step(self, sess, rev_batch, uid_batch, iid_batch, u_batch, i_batch, reuid, reiid, dropout_keep_prob, drop_his):
