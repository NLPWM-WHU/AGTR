...
class Reward(object):
    def __init__(self, lstm, update_rate, step_size):
        self.lstm = lstm
        ...
        with tf.name_scope("u_i_his_embedding"):  # users/items history reviews emb
            self.uidW = tf.identity(self.lstm.uidW)
            self.iidW = tf.identity(self.lstm.iidW)
            ...
            self.u_i_to_gen_W = tf.identity(self.lstm.u_i_to_gen_W)
            self.u_i_to_gen_b = tf.identity(self.lstm.u_i_to_gen_b)
            self.gen_input = tf.nn.tanh(
                tf.matmul(tf.concat([self.u_feas_his, self.i_feas_his], 1), self.u_i_to_gen_W) + self.u_i_to_gen_b)
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator, uid, iid):
        rewards = []
        for i in range(rollout_num):
            for given_num in range(1, len(input_x[0])):
                feed = {self.x: input_x, self.given_num: given_num, self.input_uid:uid, self.input_iid:iid}
