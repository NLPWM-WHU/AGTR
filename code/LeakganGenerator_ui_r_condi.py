...
class Generator(object):
    def __init__(self, sequence_length, num_vocabulary,
                 emb_dim, dis_emb_dim, filter_size_his, num_filters, batch_size, hidden_dim, goal_out_size,
                 goal_size, step_size, D_model, rev_num_u, rev_len_u, rev_num_i, rev_len_i,item_num, user_num, n_latent,rnn_size,
                 user_vocab_size, word_emb_size, item_vocab_size,num_filters_his, LSTMlayer_num=1,
                 l2_reg_lambda=0.0, learning_rate=0.01, start_token=0, drop_out=1):
        ...
        with tf.variable_scope('place_holder'):
            ...
            self.batch_lens = tf.placeholder(tf.int32, shape=[self.batch_size])

            self.reward = tf.placeholder(tf.float32, shape=[self.batch_size,
                                                            self.sequence_length // self.step_size])  # reward from dis
            self.reward_rate = tf.placeholder(tf.float32, shape=[self.batch_size,  self.sequence_length])  # reward from rate_pre
            self.given_num = tf.placeholder(tf.int32)
            self.drop_out = tf.placeholder(tf.float32, name="dropout_keep_prob")
            self.train = tf.placeholder(tf.int32, None, name="train")

            self.input_u = tf.placeholder(tf.int32, [None, rev_num_u, rev_len_u], name="input_u")
            self.input_i = tf.placeholder(tf.int32, [None, rev_num_i, rev_len_i], name="input_i")
            ...
            self.dropout_his = tf.placeholder(tf.float32, name="dropout_his")
            ...
            self.rate_emb_drop = tf.nn.dropout(self.rate_emb, self.dropout_his)
            self.worker_params.extend([self.rate_W, self.rate_b])
        ...
        with tf.name_scope("u_i_his_embedding"):   #users/items history reviews emb
            ...
            self.emb_u_his = tf.expand_dims(self.emb_u_his, -1)
            ...
            # self.u_his_revs = self.word_BiRNN_u_his(self.emb_u_his, batch_size*rev_num_u, word_emb_size, rnn_size, rev_len_u, self.dropout_his) #(-1, rnn_size)
            # self.i_his_revs = self.word_BiRNN_i_his(self.emb_i_his, batch_size*rev_num_i, word_emb_size, rnn_size, rev_len_i, self.dropout_his)
            ...
            self.u_feas_his, self.i_feas_his = self.u_i_his_revs_emb_cnn(filter_size_his,word_emb_size,
                                            num_filters_his, rev_len_u, rev_len_i,rev_num_u, rev_num_i,n_latent)

            self.u_feas_his_drop = tf.nn.dropout(self.u_feas_his, self.dropout_his)
            self.i_feas_his_drop = tf.nn.dropout(self.i_feas_his, self.dropout_his)

            self.u_i_to_gen_W = tf.Variable(tf.random_normal([n_latent*3, self.emb_dim], stddev=0.1))
            self.u_i_to_gen_b = tf.Variable(tf.random_normal([self.emb_dim], stddev=0.1))
            self.gen_input = tf.nn.relu(tf.matmul(tf.concat([self.u_feas_his_drop, self.i_feas_his_drop, self.rate_emb_drop], 1),
                                                  self.u_i_to_gen_W) + self.u_i_to_gen_b)
            self.worker_params.extend([self.u_i_to_gen_W, self.u_i_to_gen_b])
        ...
        with tf.variable_scope('Manager'):
            ...
            self.goal_init = tf.Variable(tf.truncated_normal([self.batch_size, self.goal_out_size], stddev=0.1),
                                         name="goal_init", )
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            with tf.variable_scope(self.scope):
                feature = self.FeatureExtractor_unit(cur_sen, self.input_uid, self.input_iid, self.drop_out, self.rate_emb)
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            sub_feature = tf.cond(((((i) % step_size) > 0)),lambda: sub_feature,lambda: (tf.cond(i > 0, lambda: sub_feature.write(i // step_size - 1,
                            tf.subtract(feature,feature_array.read(i - step_size))),lambda: sub_feature)))
            all_sub_features = tf.cond(i > 0, lambda: tf.cond((i % step_size) > 0, lambda: all_sub_features.write(i - 1,
                            tf.subtract(feature,feature_array.read(i - i % step_size))),lambda: all_sub_features.write(
                            i - 1, tf.subtract(feature,feature_array.read(i - step_size)))),lambda: all_sub_features)

            real_goal_array = tf.cond(((i) % step_size) > 0, lambda: real_goal_array,lambda: tf.cond((i) / step_size < self.sequence_length // step_size,
                    lambda: tf.cond(i > 0,lambda: real_goal_array.write((i) // step_size,real_sub_goal),lambda: real_goal_array),lambda: real_goal_array))
        ...
        _, _, self.g_predictions, _, _, _, _, _, self.feature_array, self.real_goal_array, self.sub_feature, self.all_sub_features, self.all_sub_goals = control_flow_ops.while_loop(
            ...
            body=preTrain,
            loop_vars=(
            tf.constant(0, dtype=tf.int32), tf.nn.embedding_lookup(self.g_embeddings, self.start_token), g_predictions,self.h0_worker,
        ...
        self.pretrain_goal_loss = -tf.reduce_sum(1 - tf.losses.cosine_distance(tf.nn.l2_normalize(self.sub_feature, 2),
                    tf.nn.l2_normalize(self.real_goal_array, 2), 2)) / (self.sequence_length * self.batch_size / self.step_size)
        ...
        # no pad loss
        self.len_mask = tf.sequence_mask(self.batch_lens, self.sequence_length, tf.float32)
        pre_loss_t1 = tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.log(
                tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0))
        pre_loss_t2 = tf.reshape(tf.reduce_sum(pre_loss_t1, axis=1), [self.batch_size, self.sequence_length])
        self.pretrain_worker_loss = -tf.reduce_sum(self.len_mask*pre_loss_t2)/tf.reduce_sum(tf.to_float(self.batch_lens))

        # self.pretrain_worker_loss = -tf.reduce_sum(
        #     tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.log(
        #         tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0))) / (self.sequence_length * self.batch_size)

        with tf.name_scope("Worker_rate_pre_update"):
            self.rr_worker_loss = -tf.reduce_sum(tf.reduce_sum(tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.log(
                        tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0)
                    ), 1) * tf.reshape(self.reward_rate, [-1]))
            rr_worker_opt = tf.train.AdamOptimizer(self.learning_rate)
            self.rr_worker_grad, _ = tf.clip_by_global_norm(tf.gradients(self.rr_worker_loss, self.worker_params), self.grad_clip)
            self.rr_worker_updates = rr_worker_opt.apply_gradients(zip(self.rr_worker_grad, self.worker_params))
        ...
        # loss no pad
        loss_t1 = tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.log(
            tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0))
        loss_t2 = tf.reshape(tf.reduce_sum(loss_t1, axis=1), [self.batch_size, self.sequence_length])
        self.worker_loss = -tf.reduce_sum(tf.multiply(self.Worker_Reward,self.len_mask * loss_t2)) / tf.reduce_sum(tf.to_float(self.batch_lens))

        # print self.Worker_Reward.shape
        # self.worker_loss = -tf.reduce_sum(tf.multiply(self.Worker_Reward, tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])),
        #             self.num_vocabulary, 1.0, 0.0) * tf.log(tf.clip_by_value(tf.reshape(self.g_predictions,
        #                 [-1, self.num_vocabulary]), 1e-20, 1.0)))) / (self.sequence_length * self.batch_size)
        ...
        self.pretrain_loss = -tf.reduce_sum(tf.one_hot(tf.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.log(
                tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0))) / (self.sequence_length * self.batch_size)
    ...
    def pretrain_step(self, sess, rev_batch, uid_batch, iid_batch, u_batch, i_batch, reuid, reiid, dropout_keep_prob, drop_his, r_oneh, rev_lens_batch):
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                           ...
                           feed_dict={self.x: rev_batch, self.drop_out: dropout_keep_prob, self.input_u:u_batch, self.input_i:i_batch,
                                      self.input_uid:uid_batch, self.input_iid:iid_batch, self.input_reuid:reuid, self.input_reiid:reiid,
                                      self.dropout_his:drop_his, self.rate_emb_onehot:r_oneh, self.batch_lens:rev_lens_batch})
        return outputs[1]
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.variable_scope('Worker'):
            ...
            self.Wi_worker = tf.Variable(tf.random_normal([self.emb_dim*2, self.hidden_dim], stddev=0.1))
            ...
            self.Wf_worker = tf.Variable(tf.random_normal([self.emb_dim*2, self.hidden_dim], stddev=0.1))
            ...
            self.Wog_worker = tf.Variable(tf.random_normal([self.emb_dim*2, self.hidden_dim], stddev=0.1))
            ...
            self.Wc_worker = tf.Variable(tf.random_normal([self.emb_dim*2, self.hidden_dim], stddev=0.1))
            ...
            def unit(x, hidden_memory_tm1):
                ...
                x = tf.concat([x, self.gen_input], 1)
                # Input Gate
                i = tf.sigmoid(
    ...
    def u_i_his_revs_emb_cnn(self, filter_size, word_emb_size, num_filters, rev_len_u, rev_len_i,rev_num_u, rev_num_i,n_latent):
        with tf.name_scope("user_conv"):
            ...
            u_conv = tf.nn.conv2d(self.emb_u_his,self.u_conv_W,strides=[1, 1, 1, 1],padding="VALID",name="u_conv")
            # Apply nonlinearity
            h = tf.nn.relu(tf.nn.bias_add(u_conv, self.u_conv_b), name="relu")
            # Maxpooling over the outputs
            pooled = tf.nn.max_pool(h,ksize=[1, rev_len_u - filter_size + 1, 1, 1],strides=[1, 1, 1, 1],padding='VALID',name="u_pool")
        h_pool_u = pooled
        ...
        with tf.name_scope("item_conv"):
            ...
            i_conv = tf.nn.conv2d(self.emb_i_his,self.i_conv_W,strides=[1, 1, 1, 1],padding="VALID",name="i_conv")
            # Apply nonlinearity
            h = tf.nn.relu(tf.nn.bias_add(i_conv, self.i_conv_b), name="relu")
            # Maxpooling over the outputs
            pooled = tf.nn.max_pool(h,ksize=[1, rev_len_i - filter_size + 1, 1, 1],strides=[1, 1, 1, 1],padding='VALID',name="i_pool")
        h_pool_i = pooled
        ...
        self.u_j = tf.einsum('ajk,kl->ajl', tf.nn.relu(
            tf.einsum('ajk,kl->ajl', h_pool_flat_u, self.Wau) + tf.einsum('ajk,kl->ajl', self.iid_a,
                        self.Wru) + self.bau),self.Wpu) + self.bbu  # None*u_len*1
        ...
        self.i_j = tf.einsum('ajk,kl->ajl', tf.nn.relu(tf.einsum('ajk,kl->ajl', h_pool_flat_i, self.Wai) + tf.einsum('ajk,kl->ajl', self.uid_a,
                                        self.Wri) + self.bai),self.Wpi) + self.bbi
        ...
        # self.worker_params.extend([self.u_conv_W, self.u_conv_b, self.i_conv_W, self.i_conv_b, self.Wau, self.Wru,
        #                            self.Wpu, self.bau, self.bbu, self.Wai, self.Wri,  self.Wpi, self.bai, self.bbi, self.U_to_latent_W,
        #                           self.U_to_latent_b,  self.I_to_latent_W , self.I_to_latent_b])
    ...
    def word_BiRNN_u_his(self, x, batch_size, word_emb_size, rnn_size, rev_len, dropout):
        with tf.name_scope("bi_rnn_u"):
            ...
            self.W_in_u = tf.Variable(tf.random_normal([word_emb_size, rnn_size], mean=0.0, stddev=np.sqrt(2. / (word_emb_size + rnn_size))), name='W_in')
            self.biases_in_u = tf.Variable(tf.constant(0.1, shape=[rnn_size]), dtype=tf.float32)
            x_in = tf.matmul(x, self.W_in_u) + self.biases_in_u
            ...
            with tf.variable_scope('init_name_u', initializer=tf.orthogonal_initializer()):  # 正交初始化
                LSTM_cell_fw = tf.nn.rnn_cell.LSTMCell(int(rnn_size / 2))
                LSTM_cell_bw = tf.nn.rnn_cell.LSTMCell(int(rnn_size / 2))
        ...
        self.worker_params.extend(LSTM_cell_fw.trainable_variables)
        self.worker_params.extend(LSTM_cell_bw.trainable_variables)
        self.worker_params.extend([self.W_in_u, self.biases_in_u])
        return bi_rnn_out

    def word_BiRNN_i_his(self, x, batch_size, word_emb_size, rnn_size, rev_len, dropout):
        with tf.name_scope("bi_rnn_i"):
            x = tf.reshape(x, [-1, word_emb_size])
            self.W_in_i = tf.Variable(tf.random_normal([word_emb_size, rnn_size], mean=0.0, stddev=np.sqrt(2. / (word_emb_size + rnn_size))), name='W_in')
            self.biases_in_i = tf.Variable(tf.constant(0.1, shape=[rnn_size]), dtype=tf.float32)
            x_in = tf.matmul(x, self.W_in_i) + self.biases_in_i
            x_in_drop = tf.nn.dropout(x_in, dropout)
            x_in_drop = tf.reshape(x_in_drop, [-1, rev_len, rnn_size])
            sequence_len = np.array([rev_len] * batch_size)
            with tf.variable_scope('init_name_i', initializer=tf.orthogonal_initializer()):  # 正交初始化
                GRU_cell_fw = tf.nn.rnn_cell.GRUCell(int(rnn_size / 2))
                GRU_cell_bw = tf.nn.rnn_cell.GRUCell(int(rnn_size / 2))
            (outputs, states) = tf.nn.bidirectional_dynamic_rnn(GRU_cell_fw, GRU_cell_bw, x_in_drop, sequence_len,
                                                                    dtype=tf.float32)
            bi_rnn_out = tf.concat(outputs, 2)[:, -1, :]
        self.worker_params.extend(GRU_cell_fw.trainable_variables)
        self.worker_params.extend(GRU_cell_bw.trainable_variables)
        self.worker_params.extend([self.W_in_i, self.biases_in_i])
        return bi_rnn_out
    ...
    def atten_u_i_revs(self, u_revs, i_revs, rnn_size, n_latent):
        ...
        u_feas = tf.nn.tanh(tf.matmul(u_feas, self.U_to_latent_W) + self.U_to_latent_b)
        i_feas = tf.nn.tanh(tf.matmul(i_feas, self.I_to_latent_W) + self.I_to_latent_b)
        self.worker_params.extend([self.Wau, self.Wru, self.Wpu, self.bau, self.bbu, self.Wai, self.Wri, self.Wpi, self.bai, self.bbi,
                                   self.U_to_latent_W, self.U_to_latent_b, self.I_to_latent_W, self.I_to_latent_b])
        return u_feas, i_feas