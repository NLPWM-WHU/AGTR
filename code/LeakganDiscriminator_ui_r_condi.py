...
class Discriminator(object):
    def __init__(self, sequence_length, num_classes, vocab_size,dis_emb_dim,filter_sizes, num_filters,batch_size,hidden_dim,
                 goal_out_size,step_size,user_num, item_num, n_latent, l2_reg_lambda=0.0, dropout_keep_prob=0.75):
        ...
        self.uidW = tf.Variable(tf.random_uniform([user_num, n_latent], -0.1, 0.1), name="uidW", trainable=False)
        self.iidW = tf.Variable(tf.random_uniform([item_num, n_latent], -0.1, 0.1), name="iidW", trainable=False)

        self.pad_vec = tf.get_variable(name="pad_vec",initializer=np.zeros([2, self.dis_emb_dim], np.float32), trainable=False)
        self.W_fe = tf.get_variable(name="W_fe",initializer=tf.random_uniform([self.vocab_size-1, self.dis_emb_dim], -1.0, 1.0))
        # concat
        self.W_fe = tf.concat([self.pad_vec, self.W_fe], axis=0)

        self.embedded_revs = tf.nn.embedding_lookup(self.W_fe, self.D_input_x + 1)

        self.rate_emb_onehot = tf.placeholder(tf.float32, [None, 5], name="rate_emb_onehot")   #5
        self.rate_W = tf.Variable(tf.random_normal([5, n_latent], name="rate_W"))   #5

        self.rate_b = tf.Variable(tf.constant(0.1, shape=[n_latent]), name="rate_b")
        self.rate_emb = tf.nn.tanh(tf.matmul(self.rate_emb_onehot, self.rate_W) + self.rate_b)
        ...
        with tf.name_scope('D_update'):
            ...
            with tf.variable_scope("feature") as self.feature_scope:
                D_feature = self.FeatureExtractor_unit(self.D_input_x,self.input_uid, self.input_iid, self.dropout_keep_prob, self.rate_emb)#,self.dropout_keep_prob)
    ...
    def concat_word_u_i_vec(self, u_vecs, i_vecs, rate_emb, emb_revs):
        emb_revs = tf.unstack(emb_revs, num=self.sequence_length, axis=1)
        for i in range(self.sequence_length):
            emb_revs[i] = tf.concat([emb_revs[i], u_vecs, i_vecs, rate_emb], 1)
        return tf.stack(emb_revs, axis=1)

    # This module used to Extract sentence's Feature
    def FeatureExtractor(self):
        # Embedding layer
        # scope.reuse_variables()
        def unit(Feature_input_x, input_uid, input_iid, dropout_keep_prob, rate_emb):#,dropout_keep_prob):
            with tf.variable_scope('FeatureExtractor') as scope:
                with tf.device('/cpu:0'), tf.name_scope("embedding") as scope:
                    embedded_revs = tf.nn.embedding_lookup(self.W_fe, Feature_input_x + 1)
                    u_vecs = tf.nn.embedding_lookup(self.uidW, input_uid)
                    i_vecs = tf.nn.embedding_lookup(self.iidW, input_iid)
                    D_input_con = self.concat_word_u_i_vec(u_vecs, i_vecs, rate_emb, embedded_revs)
                    embedded_chars_expanded = tf.expand_dims(D_input_con, -1)
                ...
                for filter_size, num_filter in zip(self.filter_sizes, self.num_filters):
                    with tf.name_scope("conv-maxpool-%s" % filter_size) as scope:
                        # Convolution Layer
                        filter_shape = [filter_size, self.dis_emb_dim+self.n_latent*3, 1, num_filter]
